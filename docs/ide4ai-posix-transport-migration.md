...
公共 `shell_write.eof` 已按 #2 的保守合同移除，因此本 adapter 不新增 POSIX-only VEOF port；这里的 EOF 仅表示 PTY output 端关闭。#12 的 Windows 11 Client x64 native gate 已在精确源码 `8e0626536aa1509d5919b1c1cb1a674438f21027` 上以 1/1 session、10/10 mandatory checks 和 decision `pass` 完成，证实 production Windows Profile 的控制、重建与 Job 回收路径，但不恢复无法跨平台等价的公共 EOF 输入语义。
